...
{
    public class YearlyBalanceReportPresenter : PresenterTemplate
    {
        private readonly IYearlyBalanceReportView _view;
        private readonly IPeriodicalBalanceService _service;

        public YearlyBalanceReportPresenter(IYearlyBalanceReportView view)
        {
            _view = view;
        ...
        public bool PopulateReports()
        {
            try
            {
                var yearEnd = _view.Year.GetMaximumDateOfTheYear();
                var balances = _service.GenerateBalancesFor(yearEnd);
                ...
                //            .Where(y => y.CashReceipt.ReceiptDate.Year <= year)
                //            .Sum(z => z.Amount),
                ...
                //            .Where(y => y.CashDisbursement.DisbursementDate.Year <= year)
                //            .Sum(z => z.Amount),
                ...
                //            .Where(y =>
                //                y.LoanServiceID == ServiceCodes.RegularLoan &&
                //                y.LoanReceipts.Any(z => z.CashReceipt.ReceiptDate <= yearEnd) && (
                //                //y.LoanDate.Year <= year && (
                //                y.SettlementDate != null ? y.SettlementDate.Value.Year > year : true))
                //            .SelectMany(y => y.LoanReceipts)
                //            .Sum(y => y.Amount),
                ...
                //            .Where(y =>
                //                y.LoanServiceID == ServiceCodes.RegularLoan &&
                //                y.LoanDividendAdjustments.Any(z => z.Adjustment.AdjustmentDate <= yearEnd) && (
                //                //y.LoanDate.Year <= year && (
                //                y.SettlementDate != null ? y.SettlementDate.Value.Year > year : true))
                //            .SelectMany(y => y.LoanDividendAdjustments)
                //            .Sum(y => y.Amount),
