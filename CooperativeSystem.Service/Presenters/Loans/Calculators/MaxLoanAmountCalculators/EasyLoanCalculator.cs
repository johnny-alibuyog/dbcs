...
{
    internal class EasyLoanCalculator : MaxLoanAmountCalculatorTemplate
    {
        private EasyLoanServiceRate _serviceRate;

        private EasyLoanServiceRate ServiceRate
        {
            get
            {
                if (_serviceRate == null)
                    _serviceRate = Db.GetTable<EasyLoanServiceRate>().FirstOrDefault();
                return _serviceRate;
            }
        }
