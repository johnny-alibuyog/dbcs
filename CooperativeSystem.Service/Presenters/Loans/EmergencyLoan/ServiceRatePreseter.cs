...
namespace CooperativeSystem.Service.Presenters.Loans.EmergencyLoan
{
    public class ServiceRatePreseter : PresenterTemplate
    {
        private IRepository<EmergencyLoanServiceRate> _repository;
        private EmergencyLoanServiceRate _addOnModel;
        private EmergencyLoanServiceRate _deductedModel;
        private IServiceRateView _view;

        public ServiceRatePreseter(IServiceRateView view)
        {
            _repository = new GenericRepository<EmergencyLoanServiceRate>();
            _view = view;
        }
        ...
        public void Populate()
        {
            try
            {
                IList<EmergencyLoanServiceRate> models = new List<EmergencyLoanServiceRate>();
                if (_repository.GetAll().Any())
                    models = _repository.GetAll().ToList();

                _addOnModel = models
                    .Where(x => x.LoanDeductionTypeID == LoanDeductionTypeCodes.AddOn)
                    .FirstOrDefault();

                _deductedModel = models
                    .Where(x => x.LoanDeductionTypeID == LoanDeductionTypeCodes.Deducted)
                    .FirstOrDefault();
